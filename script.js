...
// AI 분석 브리핑 생성 함수
function generateAIInsight(ratio, funding, fng) {
    const aiEl = document.getElementById('ai-briefing');
    if(!aiEl) return;
    let text = "현재 ";
    if (fng > 70) text += "<strong>탐욕 지수</strong>가 높아 고점 경계가 필요합니다. ";
    else if (fng < 30) text += "<strong>과매도 구간</strong>으로 분할 매수 기회를 탐색하세요. ";
    else text += "시장은 현재 <strong>중립적</strong>이며 에너지를 응축 중입니다. ";
    
    if (funding > 0.02) text += "펀딩비 가열로 인한 <strong>롱 스퀴즈</strong> 위험이 감지됩니다. ";
    else if (funding < 0) text += "하락 베팅이 과도하여 <strong>숏 스퀴즈</strong> 가능성이 존재합니다. ";
    
    text += ratio > 0.5 ? "개인 롱 포지션 비중이 높아 변동성에 유의하십시오." : "현재 숏 우세 장세로 반전 시 강한 상승 동력이 예상됩니다.";
...
async function updateCoinInsights() {
    try {
        ...
        const fundingRate = parseFloat(f.lastFundingRate);
        document.getElementById('btc-funding').innerText = (fundingRate * 100).toFixed(4) + '%';
        ...
        // AI 인사이트 갱신 호출
        const currentFng = parseInt(document.getElementById('fg-score').innerText) || 50;
        generateAIInsight(ratio, fundingRate, currentFng);
